   ...
   # print(query)
    ...
    if query:
        try:
            ...
            if response.status_code == 200:
                ...
                return HttpResponseServerError(f"Failed to fetch news: {data.get('message', 'Unknown error')}")

        except Exception as e:
            return HttpResponseServerError(f"Error fetching news: {str(e)}")
    else:
        # If no query parameter is provided, fetch default business news
        try:
            default_url = f'https://newsapi.org/v2/everything?q=business&apiKey={API_KEY}'
            ...
            if response.status_code == 200:
                ...
                return HttpResponseServerError(f"Failed to fetch default news: {data.get('message', 'Unknown error')}")

        except Exception as e:
            return HttpResponseServerError(f"Error fetching default news: {str(e)}")

    # Create a form instance for uploading profile picture
    form = ProfileForm()


    # Get the user's profile picture (if available)
    img = Profile.objects.filter(user=request.user).first()
    ...
    if request.method == "POST":
        ...
        if post_form.is_valid():
            ...
            post.save()
            # Redirect to homepage to display the updated feed
    ...
    return render(request, 'homepage.html', {"img": img, "form": form, "post_form": post_form, "articles": articles, "username": username, "email": email, "posts": posts})
...
def Loginpage(request):
    if request.method == 'POST':
        username = request.POST.get('username', '')  # Get username or default to empty string
        password = request.POST.get('password', '') 
        user = authenticate(request, username=username, password=password)
        if user is not None:
            login(request, user)
            return redirect('homepage')  # Redirect to index page after successful login
        else:
...
def uploadProfile(request):
    ...
    email=request.user.email
    try:
        profile = request.user.profile  # Attempt to get the user's profile
    except Profile.DoesNotExist:
        # If profile does not exist, create a new one for the user
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('uploadProfile')  # Redirect to index page after successful upload
    else:
        form = ProfileForm(instance=profile)

    return render(request, 'uploadProfile.html', {'form': form, 'username': username, 'email': email})

def PlotingData(request):
    username=request.user.username
    email=request.user.email
    import requests
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            obj = form.instance
            return render(request, "plotingData.html", {"obj": obj, "username": username, "email": email})
    else:
        form = ProfileForm()
        
    img = Profile.objects.filter(picture=True).first()
    return render(request, 'plotingData.html', {"img": img, "form": form, "username": username, "email": email})
...
def settings(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            obj = form.instance
            return render(request, "settings.html", {"obj": obj, "username": username, "email": email})
    else:
        form = ProfileForm()
        
    img = Profile.objects.filter(picture=True).first()
    return render(request, 'settings.html', {"img": img, "form": form, "username": username, "email": email})
...
def aboutus(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            obj = form.instance
            return render(request, "aboutus.html", {"obj": obj, "username": username, "email": email})
    else:
        form = ProfileForm()
        
    img = Profile.objects.filter(picture=True).first()
    return render(request, 'aboutus.html', {"img": img, "form": form, "username": username, "email": email})
...
def ticker(request):
    # ================================================= Load Ticker Table ================================================
    ticker_df = pd.read_csv('smpapp1/Data/new_tickers.csv') 
...
def predict(request, ticker_value, number_of_days):
    ...
    y = y[:-forecast_out]
    X_train, X_test, y_train, y_test = model_selection.train_test_split(X, y, test_size = 0.01) # 1% for testing_data
    ...
    ticker = pd.read_csv('smpapp1/Data/Tickers.csv')
